"""
O'yinlar — bot ichida (inline keyboard).
"""
import logging
import random
...
async def show_games_menu(message: Message) -> None:
    await message.answer("🎮 O'yinlar bo'limiga xush kelibsiz!", reply_markup=games_menu_kb())
...
async def _deduct(user_id: int, amount: int) -> bool:
    ...
    if not user or user["uc_balance"] < amount:
        return False
    await users_db.admin_adjust_balance(user_id, -amount, admin_id=None, reason="game")
    return True
...
async def baraban_start(message: Message) -> None:
    ...
    await message.answer(
        f"🎰 BARABAN\n\n💰 Balans: {bal} UC\n\nKoeffitsiyentlar: 0x, 0.5x, 1x, 1.5x, 2x, 3x, 5x, 10x, 20x\nQancha tikmoqchisiz?",
...
async def baraban_again(callback: CallbackQuery) -> None:
    ...
    await callback.message.edit_text(f"🎰 BARABAN\n\n💰 Balans: {bal} UC\n\nQancha tikmoqchisiz?", reply_markup=bet_kb("baraban"))
...
async def plinko_start(message: Message) -> None:
    bal = await _get_balance(message.from_user.id)
    await message.answer(f"🧩 PLINKO\n\n💰 Balans: {bal} UC\n\nKoeffitsiyentlar: 0.2x ~ 10x gacha\nQancha tikmoqchisiz?", reply_markup=bet_kb("plinko"))
...
async def plinko_again(callback: CallbackQuery) -> None:
    bal = await _get_balance(callback.from_user.id)
    await callback.message.edit_text(f"🧩 PLINKO\n\n💰 Balans: {bal} UC\n\nQancha tikmoqchisiz?", reply_markup=bet_kb("plinko"))
...
async def upgrade_start(message: Message) -> None:
    ...
    msg = (
        ...
        f"💰 Balans: {bal} UC\n"
        f"💵 Narxi: {cost} UC\n"
        f"📈 Muvaffaqiyat: {chance}%\n\n"
        f"⚠️ Muvaffaqiyatsiz bo'lsa, daraja 0 ga tushadi!"
    )
...
async def upgrade_do(callback: CallbackQuery) -> None:
    ...
    if success:
        ...
        msg = f"✅ Yangilash muvaffaqiyatli! Daraja: {new_level}\n💰 +{cost * 2} UC!"
    else:
        GAMES[user_id]["upgrade_level"] = 0
        msg = f"❌ Muvaffaqiyatsiz! Daraja 0 ga tushdi."
    new_bal = await _get_balance(user_id)
    msg += f"\n💰 Balans: {new_bal} UC"
    await callback.message.edit_text(msg, reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def battle_start(message: Message) -> None:
    bal = await _get_balance(message.from_user.id)
    await message.answer(
        f"⚔️ UC BATTLE\n\n💰 Balans: {bal} UC\n\nBoshqa foydalanuvchi bilan jang qiling!\nKim ko'p UC tiksa, yutish foizi shuncha yuqori.\n\nQancha UC tikmoqchisiz?",
...
async def battle_create(callback: CallbackQuery) -> None:
    ...
    await callback.message.edit_text(
        f"⚔️ UC BATTLE — Jang yaratildi!\n\n💵 Tikingiz: {amount} UC\n\n🔗 Raqib /battle_{user_id} komandasini yozib qatnashsin.",
...
async def battle_join(message: Message) -> None:
    ...
    bal = await _get_balance(user_id)
    if bal < p2_amount:
        await message.answer(f"❌ Balansingizda {p2_amount} UC mavjud emas.")
        return
    await _deduct(user_id, p2_amount)
    total = p1_amount + p2_amount
    p1_chance = (p1_amount / total) * 100
    winner = random.choices([creator_id, user_id], weights=[p1_chance, 100 - p1_chance], k=1)[0]
    if winner == creator_id:
        await _add_win(creator_id, total)
        result = f"🏆 {creator_id} yutdi! +{total} UC!"
    else:
...
async def dice_start(message: Message) -> None:
    ...
    msg = (
        f"🎲 DICE\n\n"
        f"💰 Balans: {bal} UC\n\n"
        f"1 dan 6 gacha son tanlang!\nAgar soningiz tushsa, tikish 6 barobar qaytadi!\n\n"
...
async def dice_choose_number(callback: CallbackQuery) -> None:
    ...
    await callback.message.edit_text("🎲 1 dan 6 gacha sonni tanlang:", reply_markup=InlineKeyboardMarkup(inline_keyboard=numbers))
...
async def dice_roll(callback: CallbackQuery) -> None:
    ...
    dice_faces = ["", "⚀", "⚁", "⚂", "⚃", "⚄", "⚅"]
    msg = f"🎲 DICE NATIJASI\n\nSiz: {chosen}\nTushdi: {result} {dice_faces[result]}\n\n"
    if result == chosen:
        win = amount * 6
        await _add_win(user_id, win)
...
async def dice_again(callback: CallbackQuery) -> None:
    bal = await _get_balance(callback.from_user.id)
    await callback.message.edit_text(f"🎲 DICE\n\n💰 Balans: {bal} UC\n\nQancha tikmoqchisiz?", reply_markup=bet_kb("dice"))
